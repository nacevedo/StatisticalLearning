...
for(k in 2:10){
  ...
  for(i in 1:10){
    ...
    x_train = as.data.frame(ZZ[-test_i,])
    y_train = labels[-test_i]
    
    
    CARTnum = rpart(as.factor(y_train) ~ ., data=as.data.frame(x_train), method="class")
    pred = predict(CARTnum, newdata=as.data.frame(x_test), type="class")
    t = table(y_test, pred)
    diag = diag(t)
    accuracy[i] = sum(diag/nrow(x_test))
  }
